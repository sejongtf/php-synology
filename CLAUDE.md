...
**`.gitattributes` decides what a consumer actually installs.** `composer install` defaults to `--prefer-dist`, so the GitHub archive — which honours `export-ignore` — is what lands in `vendor/`. Today that leaves `src/`, `composer.json`, both READMEs, `CHANGELOG.md` and `LICENSE`; `tests/`, `tools/`, `resources/`, `.github/`, `phpunit.xml.dist` and the agent context files are ignored (790K → 340K). **Adding a top-level file or directory means deciding which side it is on** — the test is whether it runs in a consumer's process. Verify with `git archive --format=tar HEAD | tar -t`, and note that `git archive` reads the `.gitattributes` of the commit it archives, so a staged-but-uncommitted change shows nothing. Same for tags: a rule takes effect from the next one.
