...
final class Authenticator
{
     ...
     *
     * **부르기 전에 저장소를 비우지 말 것.** 버리는 건 이 메서드가 한다 — 만료된
     * 세션에서 device token 을 챙긴 다음이라야 하기 때문이다. 미리 `forget()` 해 두면
     * 아래 조회가 빈 저장소를 읽고, 2단계 인증이 강제된 계정에서 재로그인이 OTP 를
     * 요구해 실패한다. 잠금이 필요하면 이 호출 전체를 감싸면 된다.
     */
